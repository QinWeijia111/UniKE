...
class SeracMultimodalRewriteExecutor(SeracRewriteExecutor):
    ...
    def init_model(self, model, tok, params: SERACHparams, requests: List[Dict]):
        ...
        if not self.is_init:
            if params.mixed:
                self.d_cap = torch.load(params.archive_cap, map_location='cpu')
                self.d_vqa = torch.load(params.archive_vqa, map_location='cpu')
            else:
                self.d = torch.load(params.archive, map_location='cpu')
        # self.d = torch.load(params.archive, map_location='cpu')
        if not params.mixed:
            self.alg.load_state_dict(self.d["model"], False)
            self.alg.to(torch.device(f'cuda:{params.device}'))
            self.alg.replacement.to(torch.device(f'cuda:{params.device}'))
            self.alg.classifier.to(torch.device(f'cuda:{params.device}'))
    ...
    def apply_to_model(
        ...
        **kwargs
        ...
        # if not self.is_init:
        if reload_weights:
            self.init_model(model, tok, hparams, requests)
        ...
        if hparams.mixed:
            ...
            if requests[0]['type'] == 'caption':
                self.alg.load_state_dict(self.d_cap["model"], False)
            elif requests[0]['type'] == 'vqa':
                self.alg.load_state_dict(self.d_vqa["model"], False)
            else:
                raise ValueError(f"Unknown type {requests[0]['type']} in SeracMultimodalRewriteExecutor")
            self.alg.to(torch.device(f'cuda:{hparams.device}'))
            self.alg.replacement.to(torch.device(f'cuda:{hparams.device}'))
            self.alg.classifier.to(torch.device(f'cuda:{hparams.device}'))
        ...
        edit_inner = dict_to(edit_inner, model.device)
