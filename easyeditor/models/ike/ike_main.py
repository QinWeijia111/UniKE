...
def apply_ike_to_multimodal_model(
    ...
    **kwargs: Any,    
    ...
    safe_model_name = hparams.sentence_model_name.rsplit('/', 1)[-1]
    # with open(f'{hparams.results_dir}/{hparams.alg_name}/embedding/'
    with open(f'{hparams.results_dir}/IKE/embedding/'
    ...
    hits = util.semantic_search(query_embedding, stored_embeddings, score_function=util.dot_score, top_k=hparams.k)
    assert len(hits) == 1
    # TODO: fix this to sample from top k
    hit = hits[0]
    icl_examples = [stored_sentences[hit[k]["corpus_id"]] for k in range(len(hit))]
    # random insert the new fact
    random_index = random.randrange(0, len(icl_examples))
    icl_examples.insert(random_index, f'New Fact: {new_fact}\nPrompt: {new_fact}\n\n')
    # topk_num = sims.size(-1)
    # top_k_values, top_k_indices = torch.topk(sims, topk_num, dim=-1)
    # target_topk_index = torch.multinomial(F.softmax(top_k_values, dim=-1)[0], 1)
    # cls_idxs = top_k_indices[0, target_topk_index]
    # cls_sims = sims[0, cls_idxs]
