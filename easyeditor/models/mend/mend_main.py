...
class MendMultimodalRewriteExecutor(MendRewriteExecutor):
    ...
    def init_model(self, model, tok, params: MENDMultimodalHparams, requests: List[Dict]):
        ...
        if not self.is_init:
            if params.mixed:
                self.d_cap = torch.load(params.archive_cap)
                self.d_vqa = torch.load(params.archive_vqa)
            else:
                self.d = torch.load(params.archive)
        ...
        if params.mixed:
            if requests[0]['type'] == 'caption':
                ...
                raise ValueError(f"Unknown type {requests[0]['type']} in MendMultimodalRewriteExecutor")
            
        self.alg.load_state_dict(
            {k.replace("gtn.", "mend."): v for k, v in self.d["model"].items()}
        )
        self.alg.to(torch.device(f'cuda:{params.device}'))
