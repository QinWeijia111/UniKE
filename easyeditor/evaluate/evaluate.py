...
from easyeditor.trainer.utils import dict_to, cu_del
...
def compute_icl_multimodal_edit_quality(
    ...
    if "image_rephrase" in record.keys():
        ...
        ret['image_rephrase_acc'] = rephrase_image_acc
    
    if "locality_prompt" in record.keys():
        if pre_edit:
            _, locality_output_ids, locality_output = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
        ...
        ret['locality_output_ids'] = locality_output_ids
    
    if "multimodal_locality_image" in record.keys():
        if pre_edit:
            _, locality_image_output_ids, locality_image_output = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
        ...
        ret['multimodal_locality_output_ids'] = locality_image_output_ids
...
def icl_multimodal_lm_eval(
    ...
    device = torch.device(f'cuda:{hparams.device}')
    # random choice
    if random.random() < hparams.prob_use_result:
        samples = prepare_multimodal_edit(hparams, tokenizer, target, [''.join(icl_examples) + f'{x}'], image) 
    else:
        x = f'{x}'
        random_index = random.randint(0, len(icl_examples))
        result = ' '.join(icl_examples[:random_index]) + ' ' + x + ' ' + ' '.join(icl_examples[random_index:])
        samples = prepare_multimodal_edit(hparams, tokenizer, target, [result], image)
        # samples = prepare_multimodal_edit(hparams, tokenizer, target, [''.join(icl_examples) + ' '], image) 
    
    return compute_multimodal_edit_quality(model, samples) if not is_loc else compute_multimodal_edit_quality_demo(model, samples, is_icl=True)
...
def compute_multimodal_edit_quality_demo(model, batch, is_icl=False):
    ...
    unmasked_log_probs = logits.float().log_softmax(-1).gather(-1, targ.unsqueeze(-1)).squeeze(-1)
    n_tokens = mask.float().sum()
    log_prob = (unmasked_log_probs * mask.float()).sum() / n_tokens
    # print(unmasked_log_probs)
    prob = (unmasked_log_probs.exp() * mask.float()).sum() / n_tokens
    nll_loss = -log_prob
    if is_icl:
        return acc, pred_ids.numpy(), logits_
    return acc, pred_ids.numpy(), logits_, nll_loss
...
def compute_multimodal_edit_results_demo(
    ...
    ret['rewrite_acc'], _, logits, ret['nll_loss_rewrite'] = compute_multimodal_edit_quality_demo(model, edit_inner)
    ...
    if 'locality_prompt' in record.keys():
        ...
        ret['locality_acc_unused'], ret['locality_output'] = compute_multimodal_edit_quality(model, locality)
    ...
    if 'multimodal_locality_prompt' in record.keys():
        ...
        ret['image_locality_acc_unused'], ret['multimodal_locality_output'] = compute_multimodal_edit_quality(model, m_locality)
    # Form a list of lists of prefixes to test.

    cu_del(edit_inner)
    cu_del(edit_outer)
    cu_del(edit_image_outer)
    cu_del(locality)
    cu_del(m_locality)
    
    del edit_inner
    del edit_outer
    del edit_image_outer
    del locality
    del m_locality
