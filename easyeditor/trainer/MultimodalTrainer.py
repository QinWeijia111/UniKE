...
class MultimodalTrainer(BaseTrainer):
    def __init__(self, config, train_set: Dataset, val_set: Dataset, train_func: str = None, **kwargs):
        # print(config)
        super().__init__(config, train_set, val_set, train_func, **kwargs)
    ...
    def edit_step(self, batch, training: bool, val_step: int = -1):
        self.model.train(training)
        # self.original_model.train(training)
        batch = dict_to(batch, torch.device(f"cuda:{self.config.device}"))
        # print("batch: ", batch)
        # print("batch keys: ", batch.keys())
        with torch.no_grad():
            # self.model_backup.cuda()
            base_outputs = self.model_backup(batch["loc"])
            if not isinstance(base_outputs, torch.Tensor):
                base_logits = base_outputs.logits
            else:  
                base_logits = base_outputs
                
            base_image_outputs = self.model_backup(batch["loc_image"])
        ...
        if self.config.mixed:
            # if need to reload weights according to the request type
            if batch['type'] == 'caption':
                self.model.load_state_dict(self.d_cap["model"], False)
            elif batch['type'] == 'vqa':
                self.model.load_state_dict(self.d_vqa["model"], False)
            else:
                raise ValueError(f"Unknown type {batch['type']} in MultimodalTrainer")
            self.model.to(torch.device(f'cuda:{self.config.device}'))
            # self.model.replacement.to(torch.device(f'cuda:{self.config.device}'))
            # self.model.classifier.to(torch.device(f'cuda:{self.config.device}'))
        ...
        if self.config.continuous and not training and self.config.eval_only:
            # 13 % 5
            if val_step % self.config.continuous_sample == 0:
                # restore the model
                # del self.model
                # self.model = copy.deepcopy(self.model_backup).cuda()
                self.model.model.load_state_dict(self.model_backup.model.state_dict())
                self.model.cuda()
                if self.config.alg == "SERAC_MULTI":
                    # del self.model.cache
                    del self.model.cache_inputs, self.model.cache_labels
                    self.model.cache_inputs = []
                    self.model.cache_labels = []
                print(f"Restoring model on step {val_step}")
            else:
                if self.config.alg == "SERAC_MULTI":
                    self.model.cache_inputs = copy.deepcopy(edited_model.cache_inputs)
                    self.model.cache_labels = copy.deepcopy(edited_model.cache_labels)
                self.model.model.load_state_dict(edited_model.model.state_dict())
                edited_model = edited_model.cpu()
                del edited_model
                self.model.cuda()
        torch.cuda.empty_cache()
        batch = dict_to(batch, torch.device(f"cpu"))
        del batch
    ...
    def validate(self, steps=None, log: bool = False):
        ...
        for val_step, batch in enumerate(tqdm(self.val_loader)):
            if val_step >= steps:
                break
            _, _, _, _, info_dict = self.edit_step(batch, training=False, val_step=val_step)
