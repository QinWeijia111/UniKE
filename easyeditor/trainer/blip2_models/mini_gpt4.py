...
class MiniGPT4(Blip2Base):
    ...
    def forward(self, samples):
        if samples['image'] is not None:
            image = samples["image"] # [1,3,364,364]
            img_embeds, atts_img = self.encode_img(image) # [1, 32, 4096], [1, 32]
            ...
            vqa_prompt = '###Human: <Img><ImageHere></Img> '
            img_embeds, atts_img = self.prompt_wrap(img_embeds, atts_img, vqa_prompt) # [1, 43, 4096], [1, 43]
            ...
            ).to(image.device) # input_ids: [1, 11], attention_mask: [1, 11] 
            ...
            targets = torch.cat([empty_targets, targets], dim=1) # [1, 55]
            ...
            to_regress_embeds = self.llama_model.model.embed_tokens(to_regress_tokens.input_ids) # [1, 11, 4096]
            inputs_embeds = torch.cat([bos_embeds, img_embeds, to_regress_embeds], dim=1) # [1, 55, 4096]
            attention_mask = torch.cat([atts_bos, atts_img, to_regress_tokens.attention_mask], dim=1) # [1, 55]
        else:
            text = [t + self.end_sym for t in samples["text_input"]]
        ...
        with self.maybe_autocast():
            outputs: CausalLMOutputWithPast = self.llama_model(
    ...
    def forward_with_text_ids(self, to_regress_tokens, samples, return_dict=True):
        if samples['image'] is not None:
            image = samples["image"] # [1,3,364,364]
            img_embeds, atts_img = self.encode_img(image) # [1, 32, 4096], [1, 32]
    ...
    def forward_with_embeds(self, inputs_embeds, attention_mask, labels, return_dict=True):
        with self.maybe_autocast():
            outputs = self.llama_model(
                inputs_embeds=inputs_embeds,
                attention_mask=attention_mask,
