...
class IdentityLayer(nn.Module):
    ...
    def forward(self, hidden_states, residual_before_fc):
        # return self.alpha * hidden_states + (1 - self.alpha) * residual_before_fc
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        self.mlp_handler = IdentityLayer()
    ...
    def forward(
        ...
        residual_before_fc = hidden_states
        hidden_states = self.fc1(hidden_states)
        hidden_states = self.activation_fn(hidden_states)

        hidden_states = self.fc2(hidden_states)
        
        hidden_states = self.mlp_handler(hidden_states, residual_before_fc) # used for replacement
        hidden_states = nn.functional.dropout(
            hidden_states, p=self.dropout, training=self.training
        )
...
OPT_START_DOCSTRING = r"""
    ...
    This model is also a PyTorch subclass.
