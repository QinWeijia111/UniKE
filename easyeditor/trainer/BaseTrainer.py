...
class BaseTrainer:
    def __init__(self, config, train_set: Dataset, val_set: Dataset, train_func: str = None, **kwargs):
        ...
        if 'minigpt4' in self.config.model_name.lower():
            collate_fn = train_set.collate_fn if self.alpha else train_set.collate_fn_cpu
        ...
        if not self.config.eval_only and self.config.alg!='MALMEN':
            self.OptimizerClass = getattr(torch.optim, "Adam")
        ...
        if self.config.mixed:
            self.d_cap = torch.load(self.config.archive_cap)
            self.d_vqa = torch.load(self.config.archive_vqa)
    ...
    def save_state(self, stats):
        ...
        save_path_iter = f"{self.save_path}_{self.global_iter}"
        LOG.info(f"Saving model to {save_path_iter}")
    ...
    def run(self):
        ...
        for epoch in range(self.epoches):
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                if self.global_iter % self.config.val_interval == 0 and self.global_iter >= 1000 :
                    if self.config.alg == 'MALMEN':
        ...
        if self.config.results_dir is not None:
            results_path = f"{self.config.results_dir}/json/results_{self.train_func}_{get_date()}.json"
        else:
            results_path = f"{os.getcwd()}/json/results_{self.train_func}_{get_date()}.json"

        # rename the keys in val_info
        val_info = {
            "rewrite_acc": val_info["inner/acc_val"],
            "rephrase_acc": val_info["edit/acc_val"],
            "rephrase_image_acc": val_info["image_rephrase/acc_val"],
            "locality_acc": val_info["loc/acc_val"],
            "locality_image_acc": val_info["image_loc/acc_val"],
            "num_samples": len(self.val_set),
        }
        with open(f"{results_path}", "w") as f:
            json.dump(
                {"results": val_info,
                 "config": asdict(self.config)}, f, indent=4
            )
